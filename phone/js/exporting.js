...
(function(f) {
	...
	p(A.prototype, {
		getSVG: function(b) {
			...
			h = g.exporting.sourceHeight || g.chart.height || /px$/.test(h) && parseInt(h, 10) || kuy;
			p(g.chart, {
				animation: !1,
				renderTo: b,
				forExport: !0,
			...
			d.destroy();
			o(b);
			c = c.replace(/zIndex="[^"]+"/g, "").replace(/isShadow="[^"]+"/g, "").replace(/symbolName="[^"]+"/g, "").replace(/jQuery[0-9]+="[^"]+"/g, "").replace(/url\([^#]+#/g, "url(#").replace(/<svg /, '<svg xmlns:xlink="http://www.w3.org/1999/xlink" ').replace(/ href=/g, " xlink:href=").replace(/\n/, " ").replace(/<\/svg>.*?$/, "</svg>").replace(/(fill|stroke)="rgba\(([ 0-9]+,[ 0-9]+,[ 0-9]+),([ 0-9\.]+)\)"/g, '$1="rgb($2)" $1-opacity="$3"').replace(/&nbsp;/g, "聽").replace(/&shy;/g, "颅").replace(/<IMG /g, "<image ").replace(/height=([^" ]+)/g, 'height="$1"').replace(/width=([^" ]+)/g, 'width="$1"').replace(/hc-svg-href="([^"]+)">/g, 'xlink:href="$1"/>').replace(/id=([^" >]+)/g, 'id="$1"').replace(/class=([^" >]+)/g, 'class="$1"').replace(/ transform /g, " ").replace(/:(path|rect)/g, "$1").replace(/style="([^"]+)"/g,
			function(a) {
		...
		contextMenu: function(b, a, d, c, f, h, g) {
			...
			e.container),
			w = l("div", null, p({
				MozBoxShadow: "3px 3px 10px #666",
				WebkitBoxShadow: "3px 3px 10px #666",
